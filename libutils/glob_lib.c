    ...
    void *const _data)
{
    ...
    if (n_components == 0)
    {
        ...
        Log(LOG_LEVEL_DEBUG,
            ...
            match);
        printf("Full match! Directory '%s' has matched all previous sub patterns\n", match);
    ...
    for (size_t i = 0; i < n_dirnames; i++)
    {
        ...
        if (GlobMatch(sub_pattern, dir_name))
        {
            Log(LOG_LEVEL_DEBUG,
                ...
                sub_pattern,
                dir_name);
            printf("Partial match! Sub pattern '%s' matches directory '%s'\n", sub_pattern, dir_name);
            free(short_name);
        }
...
1.             printf("Partial match! Sub pattern '%s' matches directory short name '%s' (i.e., 8.3 alias)\n", sub_pattern, NULL_TO_EMPTY_STRING(short_name));
        ...
        else
        {
            ...
            printf("Not a match! Sub pattern '%s' does not match directory '%s'\n", sub_pattern, dir_name);
    ...
    for (size_t i = 0; i < n_filenames; i++)
    {
        ...
        if (GlobMatch(sub_pattern, filename))
        {
            ...
            Log(LOG_LEVEL_DEBUG,
                ...
                filename,
                sub_pattern);
            printf("Full match! Found non-directory file '%s' where '%s' matches sub pattern '%s'\n", match, filename, sub_pattern);
            SeqAppend(data->matches, match);
        }
        else if (short_name != NULL && GlobMatch(sub_pattern, short_name))
        {
            ...
            Log(LOG_LEVEL_DEBUG,
                ...
                short_name,
                sub_pattern);
            printf("Full match! Found non-directory file '%s' where the short name '%s' (8.3 alias) matches sub pattern '%s'\n", match, NULL_TO_EMPTY_STRING(short_name), sub_pattern);
            SeqAppend(data->matches, match);
        }
        else {
            printf("Not a full match! Found non-directory file where '%s' does not match sub pattern '%s'\n", filename, sub_pattern);
        }
        free(short_name);
    }
